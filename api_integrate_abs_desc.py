...
def pipeline(ticker, product_lines, product_line_descs, fmp_api_key = "", openai_api_key = ""):
    ...
    instruction = f"""You are an expert marketing research professional who has a keen eye to identify signals and opportunities from publicly available reports and filings of companies. Your task is to understand the product offering that we are looking to sell to {ticker} and go through the entire earnings call of {ticker} meticulously to find out any announcement, mentions, strategy or initiative that could be important to determine the need for such an offering. The call transcript is attached in the vector store.
Please output the mentions, initiative or strategy in the following way:
1) The summary of the mention, initiative or strategy spoken about in the earnings call that indicates any need for the product offering.
...
2) Return all initiatives, strategies and mentions related to the product line description.
...
"""
    ...
    root_directory = f"./results_transcripts_abs_desc/{ticker}"
    ...
    with open(output_file_path, "w") as f:
        ...
        for i in range(len(product_lines)):
            print(f"-----------------{product_lines[i]}-----------------")
            thread = client.beta.threads.create(
                messages=[
                    {
                        "role": "user",
                        "content": 
f"""Search for key initiatives, strategies or mentions for the {product_lines[i]} offering.
The following is a description of the product offering:
"{product_line_descs[i]}"
...
if __name__ == "__main__":
    # Load the CSV file
    file_path = './data/ref_vaibhav_stocks.csv'
    tickers_df = pd.read_csv(file_path)

    # Extract tickers as a list
    tickers = tickers_df['ticker'].tolist()

    # Candidate labels for the departments
    # candidate_labels = ['Procurement', 'IT and Cybersecurity']
    
    # pl_path = "zluri_pl.csv"

    # candidate_labels = modified_candidate_labels_specific("2024 June Zluri POC - Eligible Departments.csv")
    data_pls_descs = pd.read_csv("./data/Generalized_Product_Line_Descriptions.csv")
    
    fmp_api_key = ""
    openai_api_key = ""

    pls = list(data_pls_descs["Product Line"])
    
    #plds = list(data_pls_descs["Value"])
    plds = list(data_pls_descs["Generalized Description"])

    for ticker in tickers:
        print(f"Ticker : {ticker}")
        response = pipeline(ticker=ticker, product_lines=pls,product_line_descs=plds, fmp_api_key=fmp_api_key, openai_api_key=openai_api_key)
        if response:
